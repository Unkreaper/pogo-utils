...
function pvpivcalc(cp_cap, poke, iv_min, s_at, s_df, s_st, max_lvl, sprod, sstats){
    ...
    ret = '<div class="row"><div class="col-2">rank</div><div class="col-1">att</div><div class="col-1">def</div><div class="col-1">hp</div><div class="col-2">lvl</div><div class="col-2">cp</div><div class="col-2">%perfect'+(sprod?' (product)':'')+(sstats?' (at, df, st)':'')+'</div></div>';
    ret += '<div class="row" style="border-top: 1px black solid; background-color: hsl('+Math.round(90*p_scale)+',100%,50%)"><div class="col-2">'+rank+'</div><div class="col-1">'+s_at+'</div><div class="col-1">'+s_df+'</div><div class="col-1">'+s_st+'</div><div class="col-2">'+(lvl/2+1)+'</div><div class="col-2">'+cp(at,df,st,cpm2[lvl])+'</div><div class="col-2">'+Math.round(stat_product/products[0][0]*100000)/1000+'%'+(sprod?' ('+Math.round(stat_product)+')':'')+(sstats?' ('+Math.round(cpm[lvl]*at*100)/100+', '+Math.round(cpm[lvl]*df*100)/100+', '+Math.floor(cpm[lvl]*st)+')':'')+'</div></div>';
    for (i=0;i<25;i++){
